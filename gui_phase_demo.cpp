...
float waterfall_mag[WATERFALL_DEPTH][MAX_FFT_SIZE / 2];
float waterfall_phase[WATERFALL_DEPTH][MAX_FFT_SIZE / 2];
int waterfall_ptr = 0;
    ...
    void draw() {
        ...
        for (int y = 0; y < WATERFALL_DEPTH; y++) {
            ...
            for (int x = 0; x < half_fft; x++) {
                ...
                float r, g, b;
                hsv_to_rgb(phase + (float)M_PI, 0.8f, mag * 2.0f, r, g, b);
                glColor3f(r, g, b);
...
void audio_timer_cb(void*) {
    if (use_mock) {
        ...
        float mock_phase[MAX_FFT_SIZE];
        float freq = 0.1f + 0.05f * sinf(angle * 0.5f);
        float phase_offset = angle;
        for (int i = 0; i < current_fft_size; i++) {
            mock_fft[2 * i] = sinf(2.0f * (float)M_PI * freq * i + phase_offset);
            mock_fft[2 * i + 1] = 0.0f;
        }
        angle += 0.05f;
        avx_fft_array(mock_fft, current_fft_size);
        avx_vector_magnitude(mock_fft, current_fft_size, mock_mag);
        avx_vector_phase(mock_fft, current_fft_size, mock_phase);

        waterfall_ptr = (waterfall_ptr + 1) % WATERFALL_DEPTH;
        for (int i = 0; i < current_fft_size / 2; i++) {
            waterfall_mag[waterfall_ptr][i] = mock_mag[i] / (current_fft_size / 2.0f);
        ...
        if (audio_capture(audio_ctx) >= 0) {
            ...
            waterfall_ptr = (waterfall_ptr + 1) % WATERFALL_DEPTH;
            for (int i = 0; i < current_fft_size / 2; i++) {
                waterfall_mag[waterfall_ptr][i] = audio_ctx->mag[i] * 5.0f;
                waterfall_phase[waterfall_ptr][i] = audio_ctx->phase[i];
            }
        }
    }
...
int main(int argc, char **argv) {
    ...
    printf("Starting AVX DSP Phase Waterfall Demo (FFT Size: %d, Mock: %s)...\n", current_fft_size, use_mock ? "Yes" : "No");
